...
describe('Complete Golf Course Management Workflow', () => {
  ...
  it('completes full workflow: Super admin signup → Course creation → Staff invite → Customer CRUD', () => {
    ...
    cy.get('[data-cy="course-form"]').within(() => {
      ...
      cy.get('[data-cy="contact-email-input"]').type(
        testData.golfCourse.contactEmail
      );
      cy.get('[data-cy="contact-phone-input"]').type(
        testData.golfCourse.contactPhone
      );
    ...
    cy.get('[data-cy="success-message"]').should(
      'contain',
      'Golf course created successfully'
    );
    cy.get('[data-cy="courses-list"]').should(
      'contain',
      testData.golfCourse.name
    );
    ...
    cy.get('[data-cy="customer-form"]').within(() => {
      ...
      cy.get('[data-cy="membership-type-select"]').select(
        testData.customer.membershipType
      );
    ...
    cy.get('[data-cy="customers-list"]').should(
      'contain',
    ...
    cy.get('[data-cy="customer-details"]').within(() => {
      cy.get('[data-cy="customer-name"]').should(
        'contain',
        `${testData.customer.firstName} ${testData.customer.lastName}`
      );
      cy.get('[data-cy="customer-email"]').should(
        'contain',
        testData.customer.email
      );
      cy.get('[data-cy="customer-phone"]').should(
        'contain',
        testData.customer.phone
      );
      cy.get('[data-cy="customer-membership"]').should(
        'contain',
        testData.customer.membershipType
      );
    });
    ...
    cy.get('[data-cy="customers-list"] [data-cy="customer-row"]').should(
      'have.length',
      1
    );
    ...
    cy.get('[data-cy="membership-filter"]').select(
      testData.customer.membershipType
    );
    cy.get('[data-cy="customers-list"]').should(
      'contain',
    ...
    cy.get('[data-cy="membership-breakdown"]').should(
      'contain',
      testData.customer.membershipType
    );
