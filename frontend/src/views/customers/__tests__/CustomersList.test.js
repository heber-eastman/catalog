    ...
    const wrapper = mount(CustomersList, {
      global: {
        ...
        stubs: {
          ...
          VSelect: { 
            template: '<select><option v-for="item in items" :key="item.value" :value="item.value">{{ item.title }}</option></select>',
          ...
          VSnackbar: { template: '<div v-if="modelValue"><slot /></div>', props: ['modelValue'] },
