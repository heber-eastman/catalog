...
describe('CoursesList', () => {
    ...
    wrapper = mount(CoursesList, {
      global: {
        ...
        stubs: {
          ...
          VSelect: {
            template:
              '<select v-model="modelValue"><option v-for="item in items" :key="item.value" :value="item.value">{{ item.title }}</option></select>',
  ...
  describe('Courses Loading', () => {
    it('loads courses on mount', async () => {
      ...
      superAdminAPI.getCourses.mockResolvedValue({
  ...
  describe('Create Course Dialog', () => {
    ...
    it('submits create form with valid data', async () => {
      ...
      expect(superAdminAPI.createCourse).toHaveBeenCalledWith(
  ...
  describe('Edit Course Dialog', () => {
    ...
    it('updates course with valid data', async () => {
      ...
      expect(superAdminAPI.updateCourse).toHaveBeenCalledWith(
        1,
        expect.objectContaining({
          name: 'Updated Course Name',
          street: '123 Updated St',
  ...
  describe('Status Change Dialog', () => {
    ...
    it('confirms status change', async () => {
      const { superAdminAPI } = await import('@/services/api');
      superAdminAPI.updateCourseStatus.mockResolvedValue({
        data: { success: true },
      });
      superAdminAPI.getCourses.mockResolvedValue({
        data: { courses: mockCourses },
      ...
      expect(superAdminAPI.updateCourseStatus).toHaveBeenCalledWith(
        mockCourses[0].id,
        'inactive'
      );
      expect(wrapper.vm.showStatusDialog).toBe(false);
  ...
  describe('Helper Functions', () => {
    it('formats location correctly', () => {
      ...
      expect(wrapper.vm.formatLocation(courseNoLocation)).toBe(
  ...
  describe('Error Handling', () => {
    ...
    it('handles status change error gracefully', async () => {
      const { superAdminAPI } = await import('@/services/api');
      superAdminAPI.updateCourseStatus.mockRejectedValue(
