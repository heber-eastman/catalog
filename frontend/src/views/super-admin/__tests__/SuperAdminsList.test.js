...
describe('SuperAdminsList', () => {
  ...
  it('renders correctly', () => {
    ...
    expect(wrapper.find('[data-cy="invite-super-admin-btn"]').exists()).toBe(true)
  })
  ...
  describe('Super Admins Loading', () => {
    it('loads super admins on mount', async () => {
      ...
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
  ...
  describe('Invite Super Admin Dialog', () => {
    ...
    it('submits invite form with valid data', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.inviteSuperAdmin.mockResolvedValue({ data: { success: true } })
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
      ...
      expect(superAdminAPI.inviteSuperAdmin).toHaveBeenCalledWith(expect.objectContaining({
        email: 'new.superadmin@example.com',
        first_name: 'New',
        last_name: 'SuperAdmin',
        phone: '555-0789'
  ...
  describe('Edit Super Admin Dialog', () => {
    it('opens edit dialog with super admin data', async () => {
      ...
      expect(wrapper.vm.editSuperAdminData.first_name).toBe(superAdmin.first_name)
      expect(wrapper.vm.editSuperAdminData.email).toBe(superAdmin.email)
    })

    it('updates super admin with valid data', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.updateSuperAdmin.mockResolvedValue({ data: { success: true } })
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
      ...
      expect(superAdminAPI.updateSuperAdmin).toHaveBeenCalledWith(1, expect.objectContaining({
        email: 'updated@example.com',
        first_name: 'Updated',
        last_name: 'Name',
        phone: '555-9999'
  ...
  describe('Deactivate Super Admin', () => {
    ...
    it('confirms super admin deactivation', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.deactivateSuperAdmin.mockResolvedValue({ data: { success: true } })
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
      ...
      expect(superAdminAPI.deactivateSuperAdmin).toHaveBeenCalledWith(mockSuperAdmins[0].id)
      expect(wrapper.vm.showDeactivateDialog).toBe(false)
  ...
  describe('Resend Invitation', () => {
    it('resends invitation successfully', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.resendInvitation.mockResolvedValue({ data: { success: true } })
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
      ...
      expect(superAdminAPI.resendInvitation).toHaveBeenCalledWith(superAdmin.id, superAdmin.email)
    ...
    it('handles resend invitation error', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.resendInvitation.mockRejectedValue(new Error('Resend failed'))
      ...
      expect(wrapper.vm.showSnackbar).toBe(true)
      expect(wrapper.vm.snackbarMessage).toContain('Failed to resend invitation')
    })
  })
  ...
  describe('Revoke Invitation', () => {
    ...
    it('confirms invitation revocation', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.revokeInvitation.mockResolvedValue({ data: { success: true } })
      superAdminAPI.getSuperAdmins.mockResolvedValue({ data: { superAdmins: mockSuperAdmins } })
      ...
      expect(superAdminAPI.revokeInvitation).toHaveBeenCalledWith(mockSuperAdmins[1].id, mockSuperAdmins[1].email)
      expect(wrapper.vm.showRevokeDialog).toBe(false)
      expect(wrapper.vm.selectedSuperAdmin).toStrictEqual(null)
    })
  })
  ...
  describe('Error Handling', () => {
    it('handles invite error gracefully', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.inviteSuperAdmin.mockRejectedValue(new Error('Invite failed'))
    ...
    it('handles update error gracefully', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.updateSuperAdmin.mockRejectedValue(new Error('Update failed'))
    ...
    it('handles deactivate error gracefully', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.deactivateSuperAdmin.mockRejectedValue(new Error('Deactivate failed'))
    ...
    it('handles revoke invitation error gracefully', async () => {
      const { superAdminAPI } = await import('@/services/api')
      superAdminAPI.revokeInvitation.mockRejectedValue(new Error('Revoke failed'))
