...
const customerSchema = Joi.object({
  first_name: Joi.string().trim().min(1).max(255)
    .messages({
  ...
  last_name: Joi.string().trim().min(1).max(255)
    .messages({
  ...
  phone: Joi.string().allow('').max(20).pattern(/^\+?[\d\s-()]*$/)
    .messages({
      'string.max': 'Phone number cannot exceed 20 characters',
      'string.pattern.base': 'Phone number can only contain numbers, spaces, and +-() characters',
    }),

  handicap: Joi.number().precision(1).min(-10).max(54).allow(null)
    .messages({
  ...
  membership_type: Joi.string().valid('Full', 'Junior', 'Senior', 'Social', 'Trial')
    .messages({
      'any.only': 'Membership type must be one of: Full, Junior, Senior, Social, Trial',
      'string.empty': 'Membership type is required',
    }),

  membership_start_date: Joi.date().iso().allow(null)
    .messages({
      'date.base': 'Please provide a valid membership start date',
    }),

  membership_end_date: Joi.date().iso().min(Joi.ref('membership_start_date')).allow(null)
    .messages({
